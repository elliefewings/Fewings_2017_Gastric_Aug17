...
kgen.df <- read.table("/scratch/medgen/users/eleanor/2015-16/1kgenomes_analysis/1000genomes_variantfilter_EUR_20170330.csv", sep = ",", header = TRUE, stringsAsFactors = FALSE)
...
#write.csv(comb.dnar, "gast_Aug17_vv_dnar_GO_v2_20171106.csv")

```

#With PRKDC variant removed
```{r fe2}

dnar2.ac <- filter(dnar.ac, SYMBOL != "PRKDC")

paths2 <- data.frame(Term=c("APOP", "DNAR"), case_AC=c(sum(apop.ac$case_AC), sum(dnar2.ac$case_AC)), kgen_AC=c(sum(apop.ac$kgen_AC), sum(dnar2.ac$kgen_AC)) )


paths2$case_without_AC <- 44 - paths2$case_AC
paths2$kgen_without_AC <- 1006 - paths2$kgen_AC

paths2$pval <- paths2 %>% apply(1, function(x){
  fishtable <- matrix(as.numeric(c(x[2:5])), nrow = 2, byrow=TRUE)
  fisher.test(fishtable)[[1]]
})

paths2$fdr.p <- signif(p.adjust(paths2$pval, method="fdr", n=nrow(paths2)), digits=2) 

kable(paths2, col.names=c("GO Term", "HDGC with variant", "Control with variant","HDGC without variant", "Control without variant", "P value", "FDR P value")) %>% column_spec(1, bold = T) %>%  kable_styling(bootstrap_options = "striped", full_width = F)
```
